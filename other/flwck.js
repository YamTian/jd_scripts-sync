...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    console.log(`------------- 共${bsxqheaderArr.length}个账号----------------\n`)
      for (let i = 0; i < bsxqheaderArr.length; i++) {
        if (bsxqheaderArr[i]) {
          message = ''
          note =''
          bsxqurl = bsxqurlArr[i];
          bsxqheader = bsxqheaderArr[i];
...
function bsxqqd(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      if (typeof $.getdata('bsxqurl') === "undefined") {
        $.msg($.name,"",'请先获取宝石星球数据!😓',)
        $.done()
      }
let url = {
        url : JSON.parse($.getdata('bsxqurl')),
        headers : JSON.parse($.getdata('bsxqhd')),
        body : ''
}
